...
#     totalIndex = 0
    ...
    def __init__(self, position, rangeLimit, comesFrom, connectionInt):
        ...
        self.dem = random.uniform(0.45,0.55)
...
#     def to_nx(self):
    ...
    def randomizePosition(self, randomRange=0.5):   #perturb by a max of length either forward or backward
...
class VertexLayer():
    ...
    def __init__(self, randomScatterInstance, directionInstance):#, idx = []):
        self.verticesOrigin = []
...
#             id = 0
        for point in randomScatterInstance.improvedPoints:
            if point[0] >=0 and point[0] <= randomScatterInstance.shape[0] and point[1] >=0 and point[1] <= randomScatterInstance.shape[1]:
                self.verticesOrigin += [VertexOrigin(point, self.rangeLimit)]#, id)]
#                     self.idx.append(id)
#                     id += 1
#         else:
#             id = idx[-1] + 1
...
#         id = idx[-1] + 1
        for vertex in self.verticesCurrent:
            if vertex.inRange:
                ...
                if vertexNext is not None:
                    self.verticesNext += vertexNext#, id)
#                     self.idx.append(id)
#                     id += 1
...
#                 print(mergeVertex.position, 'a')
                for comesFromVertex in vertex.comesFrom:
#                     print(comesFromVertex[0].position, 'b')
...
#                 if vertex.goesTo is not None:
#                     mergeVertex.goesTo = vertex.goesTo
#                     for goesToVertex in vertex.goesTo:
#                         goesToVertex[0].comesFrom += [[mergeVertex, goesToVertex[1]]]
                self.verticesNext.remove(vertex)
                vertex.removeVertex()
                self.verticesNext += [mergeVertex]
                self.verticesAll.remove(mergeVertex)
    ...
    def plotLines(self, vertices, pts, comesFrom=True, alpha=0.2, color='#ff0000'):
        if comesFrom:
            for vertex in vertices:
#                 if vertex in pts:
#                     vertex.plotComesFrom(alpha,'#0000ff')
#                 else:
            ...
            for vertex in vertices:
#                 if vertex in pts:
#                     vertex.plotGoesTo(alpha,'#0000ff')
#                 else:
...
# vex1 = VertexOrigin([1, 2],[10, 10])
# vex2 = VertexOrigin([2, 4],[10, 10], [[vex1,-1]])

# vex3 = vex2.addNextVertex()
