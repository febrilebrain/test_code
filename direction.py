...
class RoadMapBase():
    def __init__(self, path, mapType, reverse=False):
        ...
        # temp = (np.ones(self.shape)*255 - self.mapInfo, self.mapInfo)
        # print(temp[True], temp[False], temp[True].shape, temp[False].shape)
    ...
    def correctDirection(self, direction, mainDirection):
        verticalDirection= [-mainDirection[1], mainDirection[0]]    #vertical is perpendicular
...
class RoadMapRectangle(RoadMapBase):
    def __init__(self, path):
        ...
        temp = random.randint(0,359)    #subject to change
        self.mainDirection = [math.cos(math.radians(temp)), math.sin(math.radians(temp))]
        # print(self.mainDirection[0]**2 + self.mainDirection[1]**2)
...
class RoadMapCircle(RoadMapBase):
    ...
    def initCenter(self):
        ...
        # print(np.where(columnSum==np.max(columnSum)), np.where(rowSum==np.max(rowSum)),centerX, centerY)
